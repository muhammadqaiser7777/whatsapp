...
export class MessageInputComponent {
  ...
  constructor(
    ...
    // Subscribe to the selected conversation service to get the receiver ID
    this.selectedConversationService.selectedConversation$.subscribe(selectedConversation => {
      const authUser = localStorage.getItem('username') || '';
      if (selectedConversation) {
        this.receiverID = selectedConversation.user1.username === authUser
          ? selectedConversation.user2.username
          : selectedConversation.user1.username;

        // Clear the file selection and preview when the conversation changes
  ...
  if (uniqueOptions.size !== optionValues.length) {
    ...
    // Construct the poll data with question and options only
    const pollData = {
      message_type: 'poll', // The message type is 'poll'
      question: this.poll.question, // Poll question
      options: this.poll.options.map(option => option.text) // Send option text as strings
    };
  
    const token = localStorage.getItem('authToken') || '';
    const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
    
    const apiUrl = `http://127.0.0.1:8000/api/auth/send-message/${this.receiverID}`;
    
    // Send the poll data using an HTTP POST request
    this.http.post(apiUrl, pollData, { headers }).subscribe(
      (response: any) => {
        this.resetPoll(); // Reset the poll after submission
        this.closeModal();
        this.messageSent.emit(response); // Emit the actual response to notify the parent component
      },
      (error: any) => {
  ...
  setFileType(type: string) {
    switch (type) {
      case 'media':
        this.toView = '.jpeg,.png,.jpg,.gif,.mp4,.mov,.avi,.wmv'; // Allow images and videos
        break;
      case 'audio':
        this.toView = '.mp3,.wav'; // Allow audio files only
        break;
      case 'documents':
        this.toView = 'pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt'; // Allow document types
        break;
      default:
        this.toView = ''; // Default case if needed
    }
  ...
  onSend() {
    ...
    if (this.selectedFile) {
      ...
      this.http.post<any>(apiUrl, formData, { headers }).subscribe({
        next: (response) => {
          this.clearFileSelection();
          this.messageSent.emit(response); // Emit the response as new message
        },
        error: (err) => {
